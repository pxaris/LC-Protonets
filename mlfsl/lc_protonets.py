...
class LCProtonets(Protonets):
    ...
    def process_support_set(self, support_items, support_labels, device='cpu'):
        ...
        self.label_combinations_embeddings = self.compute_label_combinations_embeddings(self.support_embeddings, self.support_labels, device)
        self.prototypes, self.support_label_combinations = self.compute_prototypes(self.label_combinations_embeddings, device)
            ...
            for r in range(1, len(item_labels_indices)+1):
                ...
                for combination in labels_combinations:
                    if combination not in label_combinations_embeddings:
                        label_combinations_embeddings[combination] = torch.tensor(()).to(device)
                    label_combinations_embeddings[combination] = torch.cat([label_combinations_embeddings[combination], embedding.unsqueeze(0)])
    ...
    def expand_labels_multihot_vectors(self, items_labels):
        ...
        for i_labels in items_labels:
            ...
            item_label_combinations = self.get_all_combinations(item_labels_indices)
            ...
            item_lc_indices = [self.support_label_combinations.index(lc) for lc in item_label_combinations if lc in self.support_label_combinations]
            # create a tensor and fill the specified indices with 1
            item_expanded_labels = torch.zeros(len(self.support_label_combinations))
            item_expanded_labels[item_lc_indices] = 1
            expanded_items_labels = torch.cat((expanded_items_labels, item_expanded_labels.unsqueeze(0)), dim=0)
